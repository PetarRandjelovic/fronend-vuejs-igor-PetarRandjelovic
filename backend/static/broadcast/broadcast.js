...
function init() {
        ...
        .then( data => {
          ...
          // console.log(data);
           ...
           if(data.msg){
            ...
            const lst = document.getElementById('broadcastLst');

             lst.innerHTML += `<tr><th> ID </th> <th> dimensionID </th><th> cinemaID </th><th> movieID </th></tr>`;
            data.forEach( el => {
                lst.innerHTML += `<tr> <td> ${el.id} </td> <td> ${el.dimensionID} </td>  <td> ${el.cinemaID} </td> <td> ${el.movieID} </td> </tr>
                <a href="/admin/broadcast/updateBroadcast/${el.id}" class="btn btn-primary update"> 
                   Update
                </a>
    ...
    document.getElementById('broadcastBtn').addEventListener('click', e => {
        e.preventDefault();
        ...
        fetch('http://127.0.0.1:8090/admin/broadcast', {
           ...
           /// .then( res => res.json() )
            .then( data => {
                ...
                document.getElementById('broadcastLst').innerHTML += `<tr> <td> ${data.id} </td> <td> ${data.dimensionID} </td><td> ${data.cinemaID} </td>  <td> ${data.movieID} </td> </tr> 
                <a href="/admin/broadcast/updateBroadcast/${data.id}" class="btn btn-primary update">
                   Update
                </a>
...
function deleteUser(obj) {
    ...
    if (result) {
        ...
        var table = document.getElementById('broadcastLst');
        let i, row, colId;
        console.log(id);
