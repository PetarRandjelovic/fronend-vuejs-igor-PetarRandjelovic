        ...
        .then( data => {
          ...
          // console.log(data);
           ...
           if(data.msg){
            ...
            const lst = document.getElementById('cinemaLst');
//37 linija promeni
             lst.innerHTML += `<tr><th> ID </th> <th> Name </th> </tr>`;
            data.forEach( el => {
                lst.innerHTML += `<tr> <td> ${el.id} </td> <td> ${el.name} </td>  
                <a href="/admin/cinema/updateCinema/${el.id}" class="btn btn-primary update"> 
                   Update
                </a>
    ...
    document.getElementById('cinemaBtn').addEventListener('click', e => {
        e.preventDefault();
        ...
        fetch('http://127.0.0.1:8090/admin/cinema', {
           ...
           /// .then( res => res.json() )
            .then( data => {
                ...
                document.getElementById('cinemaLst').innerHTML += `<tr> <td> ${data.id} </td> <td> ${data.name} </td>  
                <a href="/admin/cinema/updateCinema/${data.id}" class="btn btn-primary update">
                   Update
                </a>
...
function deleteUser(obj) {
    ...
    if (result) {
        ...
        fetch('http://127.0.0.1:8090/admin/cinema/' + id, {
            method: 'DELETE',
            headers: {
                'Authorization': `Bearer ${token}`
            }
        ...
        var table = document.getElementById('cinemaLst');
        let i, row, colId;
        console.log(id);
