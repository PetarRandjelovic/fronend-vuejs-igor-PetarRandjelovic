        ...
        .then( data => {
          ...
          // console.log(data);
           ...
           if(data.msg){
            ...
            const lst = document.getElementById('dimensionLst');
//37 linija promeni
             lst.innerHTML += `<tr><th> ID </th> <th> Name </th> </tr>`;
            data.forEach( el => {
                lst.innerHTML += `<tr> <td> ${el.id} </td> <td> ${el.name} </td>  
                <a href="/admin/dimension/updateDimension/${el.id}" class="btn btn-primary update"> 
                   Update
                </a>
        ...
        fetch('http://127.0.0.1:8090/admin/dimension', {
           ...
           /// .then( res => res.json() )
            .then( data => {
                ...
                document.getElementById('dimensionLst').innerHTML += `<tr> <td> ${data.id} </td> <td> ${data.name} </td>  
                <a href="/admin/dimension/updateDimension/${data.id}" class="btn btn-primary update">
                   Update
                </a>
...
function deleteUser(obj) {
    ...
    if (result) {
        ...
        var table = document.getElementById('dimensionLst');
        let i, row, colId;
        console.log(id);
