        ...
        .then( data => {
          ...
          // console.log(data);
           ...
           if(data.msg){
            ...
            const lst = document.getElementById('genreLst');
//37 linija promeni
             lst.innerHTML += `<tr><th> ID </th> <th> Name </th> </tr>`;
            data.forEach( el => {
                lst.innerHTML += `<tr> <td> ${el.id} </td> <td> ${el.name} </td>  
                <a href="/admin/genre/updateGenre/${el.id}" class="btn btn-primary update"> 
                   Update
                </a>
        ...
        fetch('http://127.0.0.1:8090/admin/genre', {
           ...
           /// .then( res => res.json() )
            .then( data => {
                ...
                document.getElementById('genreLst').innerHTML += `<tr> <td> ${data.id} </td> <td> ${data.name} </td>  
                <a href="/admin/genre/updateGenre/${data.id}" class="btn btn-primary update">
                   Update
                </a>
...
function deleteUser(obj) {
    ...
    if (result) {
        ...
        fetch('http://127.0.0.1:8090/admin/genre/' + id, {
            method: 'DELETE',
            headers: {
                'Authorization': `Bearer ${token}`
            }
        ...
        var table = document.getElementById('genreLst');
        let i, row, colId;
        console.log(id);
