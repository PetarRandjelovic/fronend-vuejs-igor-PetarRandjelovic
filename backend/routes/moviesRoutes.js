...
const { movieSchema } = require('../routes/validation/movieSchema');
...
route.post('/', async (req, res) => {
    ...
    try{
        ...
        await movieSchema.validateAsync(dataValid, { abortEarly: false });
       Genre.findOne({ where: { id: req.body.genreID } } )
       .then(usr=>{
        if(usr!=null || usr!=undefined)
        Movie.create({ name: req.body.name , genreID:req.body.genreID})
        .then( rows => res.json(rows) )
        .catch( err => res.status(500).json(err) );
        else 
