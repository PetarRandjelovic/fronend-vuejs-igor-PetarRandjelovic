...
const { actorSchema } = require('../routes/validation/actorSchema');
...
route.get('/:firstName', (req, res) => {
    ...
    Actor.findOne({ where: { firstName: req.params.firstName } })
        .then( rows => res.json(rows) )
        .catch( err => res.status(500).json(err) );
});
...
route.post('/', async (req, res) => {
    ...
    try{
        ...
        await actorSchema.validateAsync(dataValid, { abortEarly: false });
      
        
       Actor.create({ firstName: req.body.firstName , lastName: req.body.lastName})
            .then( rows => res.json(rows) )
            .catch( err => res.status(500).json(err) );
    }
    catch(err){
